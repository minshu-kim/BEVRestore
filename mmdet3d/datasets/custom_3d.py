...
class Custom3DDataset(Dataset):
    ...
    def __init__(
        ...
        if not test_mode:
            ...
            self.data_infos = self.load_annotations(self.ann_file) #[:100]

        if test_night and test_rainy:
            rain_tokens = []
            nusc = NuScenes(version='v1.0-trainval', dataroot='data/nuscenes', verbose=False)
            for scene in nusc.scene:
                desc1 = scene['description'].split(',')[0].lower()
                try:
                    desc2 = scene['description'].split(',')[1].strip().lower()
                except:
                    continue
                if desc1 == "night" or desc1 == "rain":
                    if desc2 == "night" or desc2 == "rain":
                        rain_tokens.append(scene["token"])
            self.rain_tokens = rain_tokens

            data_infos = []
            for data in self.data_infos:
                token = data["token"]
                sample = nusc.get("sample", token)
                if sample["scene_token"] in rain_tokens:
                    data_infos.append(data)
            self.data_infos = data_infos
            nusc = None

        if test_rainy and not test_night:
            rain_tokens = []
            nusc = NuScenes(version='v1.0-trainval', dataroot='data/nuscenes', verbose=False)
            for scene in nusc.scene:
                desc = scene['description'].split(',')[0]
                try:
                    desc2 = scene['description'].split(',')[1]
                except:
                    continue
                if desc2:
                    desc2 = desc2.strip().lower() 
                if desc == "Rain": # and desc2 != "night":
                    rain_tokens.append(scene["token"])
            self.rain_tokens = rain_tokens
        ...
        if test_night and not test_rainy:
            rain_tokens = []
            nusc = NuScenes(version='v1.0-trainval', dataroot='data/nuscenes', verbose=False)
            for scene in nusc.scene:
                desc = scene['description'].split(',')[0]
