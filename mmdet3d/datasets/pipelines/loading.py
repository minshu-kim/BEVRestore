...
class LoadBEVSegmentation:
    def __init__(
        ...
        canvas_h = int(patch_h / (ybound[2] * scale))
        canvas_w = int(patch_w / (xbound[2] * scale))
    ...
    def __call__(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # labels = np.zeros((num_classes, self.canvas_size[1], self.canvas_size[0]), dtype=np.long)
...
class LoadMap:
    ...
    def __call__(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        mappings = {}
        mappings["drivable_area"] = ["drivable_area"]
        ...
        labels = np.zeros((1, *self.canvas_size), dtype=np.long)
        for layer_name in mappings["drivable_area"]:
            index = layer_names.index(layer_name)
            labels[0, masks[index]] = 1

        data["map_prior"] = labels
        
        return data


@PIPELINES.register_module()
class LoadBEVLaneSegmentation:
    def __init__(
        self,
        dataset_root: str,
        xbound: Tuple[float, float, float],
        ...
        self.vector_map = VectorizedLocalMap(
            'data/nuscenes', patch_size=self.patch_size,
            canvas_size=self.canvas_size
        )
        
    def get_semantic_map(self, vectors):
        instance_masks, forward_masks, backward_masks = preprocess_map(
            vectors, self.patch_size, self.canvas_size,
            self.NUM_CLASSES, self.thickness, self.angle_class
        )
    ...
    def __call__(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        seg, ins, dir = self.get_semantic_map(vectors)
        data["gt_lane_masks_bev"] = seg.numpy().transpose(0, 2, 1)

        return data
    

@PIPELINES.register_module()
class LoadBEVSegmentationSR:
    def __init__(
        self,
        dataset_root: str,
        xbound: Tuple[float, float, float],
        ...
        self.patch_size = (patch_h, patch_w)
        self.canvas_size = (canvas_h, canvas_w)
        self.canvas_size_x5 = (canvas_h * 5, canvas_w * 5)
        self.classes = classes
    ...
    def __call__(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        masks_x5 = self.maps[location].get_map_mask(
            patch_box=patch_box,
            patch_angle=patch_angle,
            layer_names=layer_names,
            canvas_size=self.canvas_size_x5,
        )
        masks_x5 = masks_x5.transpose(0, 2, 1)
        masks_x5 = masks_x5.astype(np.bool)

        num_classes = len(self.classes)
        labels_x5 = np.zeros((num_classes, self.canvas_size_x5[1], self.canvas_size_x5[0]), dtype=np.long)
        for k, name in enumerate(self.classes):
            for layer_name in mappings[name]:
                index = layer_names.index(layer_name)
                labels_x5[k, masks_x5[index]] = 1

        data["gt_masks_bev"] = labels.astype(np.float32)
        data["gt_masks_bev_x5"] = labels_x5
